...
def login():
    ...
    if request.method == 'POST':
        ...
        if CheckUser:
            print(CheckUser)
            if CheckUser.UserAccess == None or CheckUser.UserAccess == False:
...
def register():
    ...
    if request.method == 'POST':
        ...
        if CheckUserTypes < 1:
            UserType(TypeDescription="Admin",
                     UserCreated="Auto Generated!").save()
            UserType(TypeDescription="User",
                     UserCreated="Auto Generated!").save()
            PostType(PostTypeDescription="CSR",
                     UserCreated="Auto Generated!").save()
            PostType(PostTypeDescription="News",
                     UserCreated="Auto Generated!").save()
            PostType(PostTypeDescription="Awards",
                     UserCreated="Auto Generated!").save()
        ...
        if CountUsers > 0:
            ...
            User(UserName=Username, UserPassword=HashedPassword,
                 UserType=UserTypeObj, UserAccess=True, UserCreated=None).save()
...
@admin.route('/useraccess', methods=['GET', 'POST'])
@login_required
def useraccess():
    if request.method == 'POST':
        Id = request.form['Id']
        ...
        if Access == "Yes":
            User(id=Id).update(set__UserAccess=True)
        else:
            User(id=Id).update(set__UserAccess=False)
        Message = "Successfully Updated!"
        JsonResponse = {'Type': "Success", 'Message': Message}
        return jsonify(JsonResponse)
    return render_template('Admin/Authentication/useraccess.html')
